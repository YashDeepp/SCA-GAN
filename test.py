...
netE2 = MyEncoder(opt.input_nc, opt.output_nc, 8, opt.ngf, norm_layer=norm_layer, \
                  use_dropout=False, gpu_ids=opt.gpu_ids)
fold = opt.test_epoch
netG.load_state_dict(torch.load('/home/lixiang/lx/scagan/checkpoint/netG1_epoch_'+fold+'.weight'))
netG2.load_state_dict(torch.load('/home/lixiang/lx/scagan/checkpoint/netG2_epoch_'+fold+'.weight'))

netE.load_state_dict(torch.load('/home/lixiang/lx/scagan/checkpoint/netE1_epoch_'+fold+'.weight'))
netE2.load_state_dict(torch.load('/home/lixiang/lx/scagan/checkpoint/netE2_epoch_'+fold+'.weight'))
netE.cuda()
netE2.cuda()
netG.cuda()
netG2.cuda()
...
for i, batch in enumerate(testing_data_loader):
    ...
    # vutils.save_image(fake_s2[:,:,3:253, 28:228], output_name_A)
    torchvision.utils.save_image(fake_s2[:,:,3:253, 28:228], output_name_A)
