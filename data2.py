...
def load_inputs(imgpath,opt, if_train):
    ...
    # print imgpath
    matpath = imgpath.replace("photo/","photo_mat/")
    matpath=matpath.replace(".jpg",".mat")
    # print matpath
...
class DatasetFromFolder(data.Dataset):
    def __init__(self, opt, if_train):
        ...
        self.opt = opt
        # self.imgnames = formnames(opt.root)
        self.imgnames = os.listdir(opt.root)
        self.imgnames.sort(key= lambda x:int(x[:-4]))
    ...
    def __getitem__(self, index):
        ...
        # item = imgname.split('||')
        # inputs = load_inputs(item[0], item[0], self.opt, self.if_train)
        inputs = load_inputs(imgname,self.opt, self.if_train)
        if self.if_train:
